...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Endpoint for serving uploaded objects (images, etc)
  // No isAuthenticated middleware - authorization handled by canAccessObjectEntity
  // This allows public objects to be viewed by anyone, while private objects require auth
  app.get("/objects/:objectPath(*)", async (req, res) => {
    const userId = req.user?.id?.toString(); // May be undefined for non-authenticated users
  ...
  // Endpoint for getting upload URL for new objects
  app.post("/api/objects/upload", isAuthenticated, async (req, res) => {
    try {
      console.log("Upload request received, PRIVATE_OBJECT_DIR:", process.env.PRIVATE_OBJECT_DIR);
      const objectStorageService = new ObjectStorageService();
  ...
  // Endpoint for updating object metadata after upload (for team logos)
  app.put("/api/teams/upload-image", isAuthenticated, async (req, res) => {
  ...
  // Endpoint for updating object metadata after upload (for user profiles)
  app.put("/api/users/upload-image", isAuthenticated, async (req, res) => {
    if (!req.body.imageURL) {
      return res.status(400).json({ error: "imageURL is required" });
    }
  ...
  // Endpoint for updating object metadata after upload (for tournament images)
  app.put("/api/tournaments/upload-image", isAuthenticated, async (req, res) => {
    if (!req.body.imageURL) {
      return res.status(400).json({ error: "imageURL is required" });
    }
  ...
  // Endpoint for updating object metadata after upload (for site logos)
  app.put("/api/site/upload-image", isAdmin, async (req, res) => {
    if (!req.body.imageURL) {
      return res.status(400).json({ error: "imageURL is required" });
    }
  ...
  app.delete("/api/users/:id", isAdmin, async (req, res) => {
    try {
      ...
      // Before deleting the user, we need to clean up all related data:
      // 1. Delete all predictions made by this user
      // 2. Delete all points ledger entries for this user
      // 3. Delete all support tickets and messages created by this user
      // 4. Delete or unassign support tickets assigned to this user (if admin)
      // 5. Delete all user loves (both as lover and as loved)
      // 6. Delete all contest participant entries
      
      // Delete predictions
      await pool.query('DELETE FROM predictions WHERE user_id = $1', [id]);
      
      // Delete points ledger entries
      await pool.query('DELETE FROM points_ledger WHERE user_id = $1', [id]);
      
      // Delete ticket messages first (foreign key to tickets)
      await pool.query(
        'DELETE FROM ticket_messages WHERE ticket_id IN (SELECT id FROM support_tickets WHERE user_id = $1)',
        [id]
      );
      ...
      // Unassign support tickets that were assigned to this user (if admin)
      await pool.query('UPDATE support_tickets SET assigned_to_user_id = NULL WHERE assigned_to_user_id = $1', [id]);
      
      // Delete user loves (both as lover and as loved user)
      await pool.query('DELETE FROM user_loves WHERE lover_id = $1 OR loved_user_id = $1', [id]);
      
      // Delete contest participant entries
      await pool.query('DELETE FROM contest_participants WHERE user_id = $1', [id]);
  ...
  // Site reset endpoint - deletes all competition data but preserves user accounts
  app.post("/api/admin/site-reset", isAdmin, async (req, res) => {
    try {
      ...
      // 2. Delete all points ledger entries
      await pool.query('DELETE FROM points_ledger');
      
      // 3. Delete all contest participants
      await pool.query('DELETE FROM contest_participants');
      
      // 4. Delete all matches
      await pool.query('DELETE FROM matches');
      
      // 5. Delete all tournament-team associations
      await pool.query('DELETE FROM tournament_teams');
      
      // 6. Delete all tournaments
      await pool.query('DELETE FROM tournaments');
      
      // 7. Delete all teams (optional - can keep if you want)
      await pool.query('DELETE FROM teams');
      
      // 8. Reset all user points to 0 and clear loved_by_count, viewed_by_count
      await pool.query(`
        UPDATE users SET 
          points = 0,
          loved_by_count = 0,
      ...
      // 9. Delete all user loves
      await pool.query('DELETE FROM user_loves');
      
      // Note: User profile images, passwords, emails, security codes, and display names are preserved
      // Note: Site settings are preserved
      // Note: Support tickets are preserved
      
      res.status(200).json({ 
        message: "Site reset successful",
        deleted: {
          predictions: true,
          matches: true,
          tournaments: true,
        ...
        usersReset: {
          pointsResetToZero: true,
          lovedByCountReset: true,
          viewedByCountReset: true
        }
      });
    } catch (error) {
      console.error("Error during site reset:", error);
      res.status(500).json({ message: "Error during site reset", error: error instanceof Error ? error.message : String(error) });
    }
  });
