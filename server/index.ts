...
// Get allowed iframe embedding domains from environment or use defaults
const ALLOWED_IFRAME_PARENTS = process.env.ALLOWED_IFRAME_PARENTS 
  ? process.env.ALLOWED_IFRAME_PARENTS.split(',')
  : [
      "'self'",
      "https://pro-ace-predictions.co.uk",
      "https://*.pro-ace-predictions.co.uk" // Allow all subdomains
    ];
...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      baseUri: ["'self'"],
      formAction: ["'self'"],
      // Allow iframe embedding only from trusted WordPress domains
      frameAncestors: ALLOWED_IFRAME_PARENTS,
    },
  },
...
app.use((req: Request, res: Response, next: NextFunction) => {
  // Remove X-Frame-Options to let CSP frameAncestors handle iframe embedding
  // This allows our WordPress site while maintaining security
