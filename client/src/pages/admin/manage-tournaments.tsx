...
export default function ManageTournaments() {
  ...
  // Handle tournament image upload with object storage
  const handleImageUpload = async (result: any) => {
    ...
    if (!uploadedFileUrl) {
      ...
      toast({
        title: 'Upload Failed',
        description: 'Image upload did not complete successfully',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      const response = await fetch('/api/tournaments/upload-image', {
        method: 'PUT',
      ...
      if (!response.ok) {
        ...
        console.error('ACL setting failed:', response.status, errorText);
        toast({
          title: 'Upload Error',
          description: `Failed to process uploaded image: ${response.status}`,
          variant: 'destructive',
        });
        return;
      }
      ...
      console.log('Image processed, objectPath:', data.objectPath);
      setUploadedImageUrl(data.objectPath);
      setFormData(prev => ({ ...prev, imageUrl: data.objectPath }));
      toast({
        title: 'Image uploaded',
        description: 'Tournament image uploaded successfully to cloud storage',
      });
    } catch (error) {
  ...
  const isLoading_form = createTournamentMutation.isPending || updateTournamentMutation.isPending;
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-4">
                  ...
                  onGetUploadParameters={async () => {
                    try {
                      ...
                      if (!res.ok) {
                        ...
                        console.error('Upload URL fetch failed:', res.status, errorText);
                        toast({
                          title: 'Upload Error',
                          description: `Failed to initialize upload: ${res.status} ${res.statusText}. Please make sure you're logged in.`,
                          variant: 'destructive',
                        });
                      ...
                      console.error('Error in onGetUploadParameters:', error);
                      toast({
                        title: 'Upload Error',
                ...
                <p className="text-xs text-neutral-500">
                  Recommended size: 1920x1080px. Max 5MB. Stored permanently in cloud.
                </p>
              </div>
            </div>
