...
✓ **FIXED**: WordPress iframe embedding support (November 8, 2025)
- Updated server security headers to allow iframe embedding from WordPress domain
- Modified CSP frameAncestors to permit https://pro-ace-predictions.co.uk
- Removed conflicting X-Frame-Options DENY header
- Configurable via ALLOWED_IFRAME_PARENTS environment variable
- Maintains security by only allowing specific trusted domains
- Added Match ID column to Admin Dashboard for easy iframe embedding reference
- Created embed-optimized match detail page at /embed/matches/:id route
- Both regular browsing (/matches/:id) and iframe embedding (/embed/matches/:id) routes available

✓ **UX IMPROVEMENT**: Leaderboard rank badge visibility (November 8, 2025)
- Rank badges (crown, medals) now only appear for users with points > 0
- Prevents confusing display of 1st/2nd/3rd rankings for inactive users
- Applied to both main leaderboard and leaderboard page components

✓ **DOCUMENTATION**: WordPress embedding guide (November 8, 2025)
- Created comprehensive WORDPRESS_EMBEDDING_GUIDE.md
- Three embedding methods: iframe, shortcode, Gutenberg blocks
- Includes security considerations and responsive design tips
- Examples for tournaments, leaderboards, and individual matches
- Troubleshooting section for common issues

✓ **NEW FEATURE**: Site Reset functionality (November 8, 2025)
- Added admin-only site reset button in Site Settings
- Safely deletes all competition data: matches, predictions, tournaments, teams, contests, leaderboard points
- Preserves user accounts: emails, passwords, security codes, profile images, display names
- Includes confirmation dialog with clear communication of what gets deleted vs preserved
- Resets user points to 0 while maintaining account integrity

✓ **FIXED**: Image storage persistence and upload functionality (November 8, 2025)
- Migrated from ephemeral local storage to permanent Replit App Storage (Google Cloud)
- Team logos, tournament images, and profile pictures now persist forever
- Implemented ObjectUploader component with modern Uppy interface
- Configured "cricproace-uploads" storage bucket with proper security
- Fixed /objects/* route to allow public access for public images (tournament logos, team images)
- Public images now accessible to all users, private images remain protected
- Fixed upload button closing forms prematurely by adding type="button"
- Added comprehensive error handling with user feedback for upload failures
